...
```{r}
ewars_mort <- read.csv(here('data', 'community_mortality_20200720.csv')) %>% clean_names() 
...
```{r}
hbc_df <- read.csv(here('data', 'home_based_care_20200721.csv')) %>% clean_names() %>% 
...
symptoms_gph <- symptoms_df %>% 
  ...
  ggplot(aes(x=week,y=prop, color=interaction(sex,age_group))) +
  geom_line() +
