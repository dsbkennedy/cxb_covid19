...
```{r}
hbc_df <- read.csv(here('data', 'home_based_care_20200728.csv')) %>% 
  clean_names() %>% 
    filter(week>25) %>% 
...
#   select(timestamp,facility_name, camp, agency_in_charge, currently_accepting_patient_severity, 
#          able_to_manage_special_needs_of_covid_19_cases_tick_all_that_apply, count_beds, activity,fac_type,value) %>%
