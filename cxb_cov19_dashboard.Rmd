...
host.bins <-c(0, 100, 250, 500, 1000, 2500)

host_pal <- colorBin( "YlOrRd", bins=host.bins, na.color = "#aaff56")
  ...
  addPolygons(data=case_shp_host,
              color="black",
              fillColor = ~ host_pal(total_cases),
              stroke = TRUE,   
              smoothFactor = 0.2,
              popup=popup,
              weight = 1,
              fillOpacity = .6) %>% 
  addLegend("bottomright", pal = host_pal, values = ~host_pal, title = "Number of cases")
...
fdmn_pal <- colorBin( "YlOrRd", bins=fdmn.bins, na.color = "grey")
...
popup <- paste(
  ...
  addPolygons(data=case_shp_fdmn,
              color="black",
              fillColor = ~ fdmn_pal(total_cases),
              stroke = TRUE,   
              smoothFactor = 0.2,
              popup=popup,
              weight = 1,
              fillOpacity = .6) %>% 
  addLegend("bottomright", pal = fdmn_pal, values = ~host_pal, title = "Number of cases")
...
breaks <- c(-Inf,5,18,59,Inf)
labs <- c('Under 5', '5 to 18', '18 to 59', '60 and over')
...
#   ggplot(aes(x=fct_rev(camp), y=estimate)) +
#   geom_point() +
#   coord_flip() +
...
#   labs(x='Camp', 
#        y='% COVID-19 + samples', 
#        title='Test postivity by camp', 
#        subtitle='Average (mean) with 95% confidence intervals', 
...
tests_gph <- ari_ili_df %>%  
  ...
  labs(x='Age group', 
       y='% COVID-19 + samples', 
       title='Test postivity by age group', 
       subtitle='Average (mean) with 95% confidence intervals', 
       caption='Data source:ARI/ILI linelist') +
  scale_y_continuous(labels = scales::percent)  +
  theme(legend.position='top')
...
age_group_pop <- population %>%  select(-c(total_families, total_individuals)) %>% 
 ...
 tests_age_group_gph <- tests_age_group_df %>% 
  ...
  mutate(age_group=factor(age_group, levels=c('Under 5', '5 to 18', '18 to 59', '60 and over'))) %>% 
...
```{r}
ewars_mort <- read.csv(here('data', 'community_mortality_20200804.csv')) %>% clean_names() %>% 
  ...
  mutate(age_group=case_when(age_in_years<5 ~ 'Under 5',
                             ...
                             age_in_years>=15 & age_in_years<60 ~ '15 to 60',
                             age_in_years>=60 ~'60 and over')) %>% 
  mutate(age_group=factor(age_group, levels=c('Under 5', '5 to 14', '15 to 60', '60 and over', NA))) 
...
```{r}
hbc_df <- read.csv(here('data', 'home_based_care_20200805.csv')) %>% 
  clean_names() %>% 
...
```{r}
# hhvisit_camp_gph <- hbc_df %>% 
#   select(week, camp, c1_total_hh_visited_week) %>% 
#   ungroup() %>% 
#   filter(week>25) %>% 
#   mutate(camp=gsub('camp_', '', camp)) %>% 
#   mutate(camp_number=str_extract(camp, "[[:digit:]]+")) %>% 
...
#   geom_line() +
#   # scale_x_continuous(breaks = c(25, 26, 27, 28, 29)) +
#   labs(x='Week', y='Households visited', title='Households visited- by camp') +
#   theme_minimal() +
#   facet_wrap(~camp)

# p <- ggplotly(hhvisit_camp_gph + ylab(" ") + xlab(" "))
# x <- list(
#   title = "Week"
# )
# y <- list(
...
#hhvisit_camp_gph
  ...
  camp_activity <- hbc_df %>% 
    ...
    #mutate(prop_visited=(round(prop_visited,2))*100)
    # mutate(prop_visited=scales::percent(prop_visited, accuracy=1), 
    #        prop_mild=scales::percent(prop_mild,accuracy=1))
...
prop_visit_camp_gph <-  ggplot(camp_activity, aes(x=week, y=prop_visited)) +
    geom_col() +
    #geom_line() +
    #geom_text(aes(label=prop_visited)) +
    #theme_minimal() +
    scale_y_continuous(labels = scales::percent) +
    facet_wrap(~camp) +
    theme(legend.position = "none") +
...
ggplotly(symptoms_gph)

# total_consultations_gph <- symptoms_df %>%
...
# 
# # #ggplotly(total_consultations_gph)
# # 
# # total_consultations_gph
# 
# q <- ggplotly(total_consultations_gph + ylab(" ") + xlab(" "))
# 
# x2 <- list(
#   title = "Week"
# )
...
# cbs_sex_gph <-  symptoms_df %>% 
#   ungroup() %>% 
#   group_by(severity,week, sex) %>% 
#   summarise(total_cases=sum(value,na.rm=TRUE)) %>% 
#   group_by(severity,week) %>% 
#   mutate(prop=total_cases/sum(total_cases)) %>% 
#   ggplot(aes(x=week,y=prop, color=sex)) +
#   geom_line() +
#   facet_wrap(~severity, ncol=2) +
#   theme_minimal() +
#     labs(x='Week', y='Proportion of cases', color='Sex') +
#   scale_y_continuous(labels = scales::percent, limits=c(0,1))
# 
# 
# cbs_age_gph <- symptoms_df %>% 
#   ungroup() %>% 
#   group_by(severity,week, age_group) %>% 
#   summarise(total_cases=sum(value,na.rm=TRUE)) %>% 
#   group_by(severity,week) %>% 
#   mutate(prop=total_cases/sum(total_cases)) %>% 
#   ungroup() %>% 
#   ggplot(aes(x=week,y=prop, color=age_group)) +
#   geom_line() +
#   facet_wrap(~severity, ncol=2) +
#   theme_minimal() +
#   labs(x='Week', y='Proportion of cases', color='Age group') +
#   scale_y_continuous(labels = scales::percent, limits=c(0,1))
# 
# cbs_sex_gph / cbs_age_gph & theme(legend.position = "top")
...
<iframe width='100%' height='100%' src="https://app.powerbi.com/view?r=eyJrIjoiY2YyMDY3OWItZWU1My00ZDRkLWE2MWMtMjc2YWJlYTkzNDY3IiwidCI6IjE1MjZjY2Q5LWQ2YjktNDM2MC1iOTk5LTlmMmY4MjE4M2RjNiIsImMiOjl9"  >
</iframe>
...
dru_gt_table <- dru_raw %>%
  ...
  prop_filled_iso=(ifelse(number_of_currently_filled_isolation_beds_non_sari>0, (number_of_currently_filled_isolation_beds_non_sari/number_of_functional_sari_beds),NA)),
  camp= gsub("Camp", "", camp), 
