...
An estimated 745,000 Rohingya people have crossed from Myanmar into Cox’s Bazar, Bangladesh since August 2017, joining approximately 212,000 others who had fled in earlier waves of displacement. The living conditions of the refugee population remain extremely vulnerable. This is explained by many factors including the clustering of populations in hastily constructed settlements clinging to steep, rapidly deforested hillsides. Overcrowding, insufficient food, and poor water, sanitation and hygiene (WASH) present serious health threats. Since 2017 there have been multiple outbreaks of epidemic-prone diseases including cholera, diphtheria and measles that highlight the vulnerability of the Rohingya population.  
...
Data have been brought together from numerous sources to build up a picture of COVID-19 in Cox's Bazar.

This is the landing page and it contains a summary of essential information on COVID-19. Additional resources can be fouund in the box on the bottom left portion of this page. 

There are individual tabs for the "Host Community" & the "FDMN/Rohinyga Community". These tabs contain indicators which may be useful for understanding how COVID-19 is developing.   

In the "Epidemiology" tab, data collected using GoData is presented to gain further insight into how COVID-19 is presenting in the Rohinyga population. 

The final tab contains data on healthcare facility capacity for both SARI-ITC & Quarantine facilities. 


```{r test, eval=FALSE, include=FALSE}
### Tested Host (24 hours & Total)
...
flexdashboard::valueBox(
  format(paste(total_tests_host,'(', total_tests_host_24h, ')     ', total_tests_fdmn, '(',total_tests_fdmn_24h,')'), big.mark = ","),
  color = "#d6804b",
  icon = "fas fa-vials"
)
...
```{r dead, eval=FALSE, include=FALSE}
### Dead (24 hours & Total)
...
flexdashboard::valueBox(
  format(paste(total_deaths_host,'(', total_deaths_host_24h, ')     ', total_deaths_fdmn, '(',total_deaths_fdmn_24h,')'), big.mark = ","),
  color="#D55E00"
  )
...
```{r case, eval=FALSE, include=FALSE}
### Cases Total (Today)
...
flexdashboard::valueBox(
  format(paste(total_cases_host,'(', total_cases_host_24h, ')     ', total_cases_fdmn, '(',total_cases_fdmn_24h,')'), big.mark = ","),
  icon = "fas fa-user-md",
  color= "light blue"
)
...
table_calc_comb %>% 
  ...
  fmt_percent(
    columns = vars(case_growth, cfr,test_pos),
    decimals = 1
  ) %>% 
  fmt_number(
    columns=vars(total_cases_pm, total_cases_pm_7day, total_deaths_pm,total_deaths_pm_7day, total_tests_pm,total_tests_pm_7day),
    decimals = 1
  )
...
### Additional Resources
  ...
  + [Field manuals](https://ln2.sync.com/dl/77f93df70/czt8mutv-8rcug6ge-erffrmd5-5s9i76c3)
  
  + [Infographics](https://ln2.sync.com/dl/8da6ec810/f8q6b3c7-vgnyxh76-cepdkgqu-e2q4d9na)
  
  + [Healthcare facility monitoring (Live Dashboard)](https://worldhealthorg.shinyapps.io/cxb_dru_dashboard)
...
knitr::include_graphics(here('data', 'who_logo.png'), dpi = 100)
...
cxb_cases_subloc <- all_cases_linelist %>% 
  group_by(population_group,date_of_case_detection,upazilla, camp) %>% 
  summarise(n = n(),.groups = 'drop') %>% 
  select(date=date_of_case_detection, population_group,upazilla, camp, new_cases=n)
...
cxb_deaths_subloc <- all_cases_linelist %>% 
  ...
  group_by(population_group,date_of_case_detection,upazilla, camp) %>% 
  summarise(n = n(),.groups = 'drop') %>% 
  select(date=date_of_case_detection, population_group,upazilla, camp, new_deaths=n)

cxb_cases_deaths_subloc <- cxb_cases_subloc %>% 
  full_join(cxb_deaths_subloc, by=c('population_group', 'date', 'upazilla', 'camp')) %>% 
...
summaryTable_host <- table_calc_comb_subloc %>% 
  ...
  fmt_number(
    columns=vars(total_cases_pm, total_cases_pm_7day, total_deaths_pm,total_deaths_pm_7day),
    decimals = 1
  ) %>% 
...
#   addPolygons(fillColor = ~pal_fdmn_case(total_cases),
#               weight = 2,
#               opacity = 1,
#               color = "white",
#               dashArray = "3",
#               fillOpacity = 0.7,
#               label = labels_fdmn_case,
#               labelOptions = labelOptions(
#                 style = list("font-weight" = "normal", padding = "3px 8px"),
#                 textsize = "15px",
#                 direction = "auto"), 
#               group="Cases")   %>% 
#   addLegend(pal = pal_fdmn_case, 
#             values = ~total_cases, opacity = 0.7,
#             position = "bottomright", title="Confirmed cases (n)", group="Cases") %>% 
#   addPolygons(fillColor = ~pal_fdmn_death(total_deaths),
#               weight = 2,
#               opacity = 1,
#               color = "white",
#               dashArray = "3",
#               fillOpacity = 0.7,
#               label = labels_fdmn_death,
#               labelOptions = labelOptions(
#                 style = list("font-weight" = "normal", padding = "3px 8px"),
#                 textsize = "15px",
#                 direction = "auto"), 
#               group="Deaths")   %>% 
#   addLegend(pal = pal_fdmn_death, 
#             values = ~total_deaths, opacity = 0.7,
#             position = "bottomright", title="Confirmed deaths (n)", group="Deaths") %>%
#   addLayersControl(
...
FDMN/Rohingya community {data-orientation=columns}
=====================================  
...
case_shp_fdmn <- table_calc_comb_subloc %>% 
  ...
  mutate(code=as.numeric(location)) %>% 
...
map_fdmn <- case_shp_fdmn %>%
  ...
  tm_shape(.) +
  tm_polygons("total_cases", title="Confirmed COVID-19 cases", palette='YlOrRd', id="confirmed", alpha=0.5, colorNA=NULL) +
  tm_text("camp", root=5) +
  tm_legend(text.size=1,
            title.size=1.2,
            position = c("left","bottom"),
...
### Cases by date and sex {data-height=350}
...
table1(~ age_years  + sex + risk_level +onset_sample | outcome , data=godata_clean, topclass="Rtable1-zebra",footnote="Data source: GoData") 
...
SARI ITC & Isolation  {data-orientation=rows data-navmenu="Healthcare facility monitoring" data-icon="fa-list"}
=====================================  


```{r dru_hf}

tableFormat <- gsheet_data$dru %>%
  clean_names() %>% 
  select(facility_name, camp, agency_in_charge, currently_accepting_patient_severity, able_to_manage_special_needs_of_covid_19_cases_tick_all_that_apply, contains('sari'), contains('isolation')) %>%
  pivot_longer(-c(facility_name, camp, agency_in_charge, currently_accepting_patient_severity,able_to_manage_special_needs_of_covid_19_cases_tick_all_that_apply)) %>%
  ...
  select(facility_name, camp, agency_in_charge, currently_accepting_patient_severity, able_to_manage_special_needs_of_covid_19_cases_tick_all_that_apply, count_beds, activity,fac_type,value) %>%
  ...
  mutate(paed=ifelse(grepl('Pead', able_to_manage_special_needs_of_covid_19_cases_tick_all_that_apply) , '+', '')) %>%
  ...
  select(facility_name, camp,mild:preg, FUNCTIONAL_SARI, FILLED_SARI,prop_filled_sari, DISCHARGES_SARI,
...
#Cox's Bazar is 5 hours ahead of UK
cxb_time <- now("GMT") + lubridate::hours(6)

last_upd <- gsheet_data$dru %>%
  clean_names() %>% 
  select(facility_name, timestamp)
#mutate(time_entry=as.POSIXlt(timestamp,format="%Y-%m-%dT%H:%M:%S")) %>%
#mutate(time_since_upd=cxb_time-timestamp) %>%
# mutate(hours_since_upd=round(time_since_upd,0)) %>%
#select(facility_name,hours_since_upd)
...
Quarantine   {data-orientation=rows data-navmenu="Healthcare facility monitoring" data-icon="fa-list"}
=====================================  
