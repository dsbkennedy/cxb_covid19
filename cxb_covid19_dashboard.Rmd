...
bgd_data <- fread('https://covid.ourworldindata.org/data/owid-covid-data.csv') %>% 
  filter(iso_code=='BGD') %>%        
  mutate(date_format=ymd(date)) %>% 
  select(date=date_format, new_tests,new_cases,total_deaths_per_million,population) %>% 
  mutate(population_group='Bangladesh')
...
cxb_deaths_per_million <- all_cases_linelist %>% 
  ...
  select(population_group, deaths_per_million)

deaths_per_million <- bgd_data %>% 
  select(date, total_deaths_per_million) %>% 
  summarise(deaths_per_million=last(total_deaths_per_million)) %>% 
  mutate(population_group='Bangladesh',
...
comp_table %>% 
  ...
  tab_header(
    title = "Comparing COVID-19 indicators",
    subtitle = "All figures are standardised to per million people"
  ) %>% 
  cols_label(
    tests_per_million = "Total tests",
    cases_per_million = "Total cases",
    deaths_per_million = "Total deaths"
  ) %>% 
  tab_style(
    style = list(
      cell_borders(
...
This table shows that the number of tests per million people is substantially lower for the Rohingya Refugee/FDMN population compared to both the host community and to all of Bangladesh. 


This low level of testing may also explain the low number of cases per million people among the Rohingya Refugee/FDMN population compared to the other population groups
