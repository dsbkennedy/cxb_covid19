...
cxb_data <- tests_data %>%  
  filter(!date %in% c('NULL', 'Total')) %>% 
  mutate(date_format=excel_numeric_to_date(as.numeric(date)))  %>% 
  select(date=date_format, fdmn,fdmn_positive, host,host_positive) %>% 
  pivot_longer(-date) %>% 
  ...
  select(date,new_tests=Test,new_cases=Case, population_group) %>% 
...
#   filter(iso_code=='BGD') %>%        
#   mutate(date_format=ymd(date)) %>% 
#   select(date=date_format, new_tests,new_cases,population) %>% 
#   mutate(population_group='Bangladesh')
# 
# cxb_bgd_data <- bgd_data %>% 
#   bind_rows(cxb_data)
# 
# 
# tests_per_million <- cxb_bgd_data %>% 
...
#   select(population_group, tests_per_million)
# 
# cases_per_million <- cxb_bgd_data %>% 
#   group_by(population_group,population) %>% 
#   summarise(total_cases= sum(new_cases, na.rm=TRUE)) %>% 
#   mutate(cases_per_million=(total_cases/population)*1000000) %>% 
#   mutate(cases_per_million=round(cases_per_million,0)) %>% 
#   select(population_group, cases_per_million)
...
bgd_data <- fread('https://covid.ourworldindata.org/data/owid-covid-data.csv') %>% 
  filter(iso_code=='BGD') %>%        
  mutate(date_format=ymd(date)) %>% 
  select(date=date_format, new_tests,new_cases,total_deaths, total_deaths_per_million,population) %>% 
  mutate(population_group='Bangladesh')
...
tests_per_million <- cxb_bgd_data %>% 
  group_by(population_group,population) %>% 
  summarise(total_tests= sum(new_tests, na.rm=TRUE),
            today_tests=last(na.omit(new_tests))) %>% 
  mutate(tests_per_million=(total_tests/population)*1000000) %>% 
  mutate(tests_per_million=round(tests_per_million,0)) %>% 
  select(population_group, total_tests, tests_per_million,today_tests)

cases_per_million <- cxb_bgd_data %>% 
  group_by(population_group,population) %>% 
  summarise(total_cases= sum(new_cases, na.rm=TRUE),
            today_cases=last(na.omit(new_cases))) %>% 
  mutate(cases_per_million=(total_cases/population)*1000000) %>% 
  mutate(cases_per_million=round(cases_per_million,0)) %>% 
  select(population_group, total_cases, cases_per_million,today_cases)
...
cxb_deaths_per_million <- all_cases_linelist %>% 
  filter(x30_day_outcome=='Death') %>% 
  group_by(population_group) %>% 
  summarise(n = n(),.groups = 'drop') %>% 
  mutate(population=case_when(population_group=='Rohingya refugee/FDMN' ~ fdmn_population, 
                              TRUE ~ host_population)) %>% 
  mutate(deaths_per_million=(n/population)*1000000) %>% 
  mutate(deaths_per_million=round(deaths_per_million,0)) %>% 
  select(population_group, total_deaths=n, deaths_per_million)

deaths_per_million <- bgd_data %>% 
  select(date, total_deaths, total_deaths_per_million) %>% 
  summarise(total_deaths=last(total_deaths),
    deaths_per_million=last(total_deaths_per_million)) %>% 
  mutate(population_group='Bangladesh',
         deaths_per_million=round(deaths_per_million,0)) %>% 
  bind_rows(cxb_deaths_per_million)



comp_table <- tests_per_million %>% 
  left_join(cases_per_million, by='population_group') %>% 
    left_join(case_growth_rate, by='population_group') %>% 
  left_join(deaths_per_million, by='population_group')

comp_table %>% 
  ungroup() %>%
  select(population_group, total_cases, cases_per_million,case_growth, today_cases, total_deaths, deaths_per_million, total_tests, tests_per_million, today_tests) %>% 
  mutate(cfr=total_deaths/total_cases) %>% 
  mutate(test_positivity=total_cases/total_tests) %>% 
  ...
  cols_label(
    ...
    total_cases = "Total",
    total_deaths = "Total",
    tests_per_million = "Per million",
    cases_per_million = "Per million",
    deaths_per_million = "Per million", 
    today_cases = "Last 24 hours", 
    today_tests = "Last 24 hours",
  ...
  tab_spanner(
    label = "Cases",
    columns = vars(total_cases, cases_per_million,today_cases,case_growth)
  ) %>% 
  tab_spanner(
    label = "Deaths",
    columns = vars(total_deaths, deaths_per_million, cfr)
  ) %>% 
  tab_spanner(
    label = "Tests",
    columns = vars(total_tests, tests_per_million,today_tests,test_positivity)
  ) %>% 
  tab_style(
    style = list(
      cell_borders(
  ...
  fmt_percent(
    columns = vars(case_growth, cfr,test_positivity),
    decimals = 1
  ) 
...
map_fdmn <- case_shp_fdmn %>% 
  mutate(camp=gsub('Camp', '', new_camp_n)) %>% 
  tm_shape(.) + 
  tm_polygons("confirmed", title="Confirmed COVID-19 cases", palette='YlOrRd', id="confirmed") +   
  tm_text("camp", root=5) +
  tm_legend(text.size=1,
            title.size=1.2,
            position = c("left","bottom"), 
