...
sheet_names <- c('host_fdmn_append', 'ari_ili', 'testing', 'dru', 'quarantine')
...
```{r}
 ...
 valueBox(
  format(paste0('Rohingya refugee/FDMN'), big.mark = ","),
  icon= "",
  color= "#4472C4"
)
...
fdmn_deaths_7day <- table_calc_comb %>%  filter(population_group=='Rohingya refugee/FDMN') %>%  pull(total_deaths_7day)
 ...
 valueBox(
  format(paste(fdmn_deaths,' (', fdmn_deaths_7day, ')'), big.mark = ","),
  icon = "fas fa-procedures",
  color= "#4472C4"
)
...
table_calc_comb %>% 
  ungroup() %>%
  #select(-c(total_deaths_pm,total_deaths_pm_7day))
...
test_positivity_gph <- test_nationality %>% 
  ...
  group_by(population_group) %>% 
  mutate(tests_roll=roll_mean((Test),14, na.rm=TRUE, align="right", fill = NA)) %>% 
  mutate(cases_roll=roll_mean((Case),14, na.rm=TRUE, align="right", fill = NA)) %>% 
  ...
  labs(caption="Data source: Lab data",
       x = "Date",
       y = "Test positivity (%) (14-day average)",
...
summaryTable_fdmn <- table_calc_comb_subloc %>% 
  ...
  mutate(camp=as.numeric(location)) %>% 
  arrange(camp) %>% 
  select(-c(population_group,camp,total_deaths_pm,total_deaths_pm_7day)) %>% 
  gt() %>% 
  # tab_header(
...
godata_clean <- godata_wide %>% 
  ...
  mutate(outcome = case_when(
            ...
            grepl('death', questionnaire_answers_outcome_2, ignore.case=TRUE) ~ "dead"
